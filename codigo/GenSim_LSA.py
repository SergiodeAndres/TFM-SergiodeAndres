...
def realizar_analisis(corpus, topic, analisis, num_temas=5, num_palabras=10):
        ...
        lsa_topics = LsiModel(matriz_termino_doc, num_temas, id2word=diccionario, random_seed=1)
        ...
        dfi.export(df_combinado, f'result_files\\GenSim_LSA\\GenSim_LSA_{topic}_{analisis}_{len(corpus_preprocesado)}.png')
...
if tema == "1":
    ...
    if tipo_analisis == "1":
        ...
        for trimestre, docs_trimestre in full_corpus.items():
             resultados = realizar_analisis(docs_trimestre, "ElonMusk", "trimestre_" + trimestre)
    ...
    if tipo_analisis == "1":
        ...
        full_corpus = preprocessing_file.obtener_corpus_por_fecha(subreddits)
        for trimestre, docs_trimestre in full_corpus.items():
             resultados = realizar_analisis(docs_trimestre, "SpaceX", "trimestre_" + trimestre)
    elif tipo_analisis == "3":
         for subreddit in subreddits:
            corpus = preprocessing_file.obtener_corpus(f'output_files\\{subreddit}.csv')
