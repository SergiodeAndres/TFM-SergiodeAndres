...
def realizar_analisis(corpus, topic, analisis, num_temas=5, num_palabras=10):
        ...
        lda_model = LatentDirichletAllocation(n_components=num_temas, random_state=1)
        lda_model.fit_transform(Tf_idf_score)

        vocabulario = Tf_idf_vectorizer.get_feature_names_out()
        lista_dfs = []
        for i, componente in enumerate(lda_model.components_):
        ...
        dfi.export(df_combinado, f'result_files\\ScikitLearn_LDA\\ScikitLearn_LDA_{topic}_{analisis}_{len(corpus_preprocesado)}.png')
