...
def realizar_analisis(corpus, topic, analisis, num_temas=5, num_palabras=10):
        ...
        lsa_model = TruncatedSVD(n_components=num_temas, random_state=1)
        lsa_model.fit_transform(Tf_idf_score)

        vocabulario = Tf_idf_vectorizer.get_feature_names_out()
        lista_dfs = []
        for i, componente in enumerate(lsa_model.components_):
            tupla_componente_palabra = zip(vocabulario, componente)
        ...
        dfi.export(combined_df, f'result_files\\ScikitLearn_LSA\\ScikitLearn_LSA_{topic}_{analisis}_{len(corpus_preprocesado)}.png')



print("Tema a analizar (1. Elon Musk, 2:SpaceX): ")
tema = input()
print("Tipo de análisis (1. Cuerpo completo, 2. Cuerpo completo por trimestre, 3. Cuerpo por subreddit): ")
tipo_analisis = input()
...
if tema == "1":
    ...
    if tipo_analisis == "1":
        ...
        for trimestre, docs_trimestre in full_corpus.items():
             realizar_analisis(docs_trimestre, "ElonMusk", "trimestre_" + trimestre)
    ...
    if tipo_analisis == "1":
        ...
        full_corpus = preprocessing_file.obtener_corpus_por_fecha(subreddits)
        for trimestre, docs_trimestre in full_corpus.items():
             realizar_analisis(docs_trimestre, "SpaceX", "trimestre_" + trimestre)
    elif tipo_analisis == "3":
         for subreddit in subreddits:
            corpus = preprocessing_file.obtener_corpus(f'output_files\\{subreddit}.csv')
